...
def Linear_SVC(train_x, tran_y, test_x):
    train_re = OneVsRestClassifier(LinearSVC(random_state=0, penalty='l2', C=54)).fit(train_x, tran_y)
    y_pred = train_re.predict(test_x)
    return y_pred

def KNN(train_x, tran_y, test_x):
    neigh = KNeighborsClassifier(n_neighbors=86, weights='distance')
...
def XGBoosting(train_x, tran_y, test_x):
    XGB = XGBClassifier(random_state=0, n_estimators=801, max_depth=1)
...
def RandomForest(train_x, tran_y, test_x):
    forest = RandomForestClassifier(random_state=0, n_estimators=301, max_depth=11)
...
def MLP_Classifier(train_x, tran_y, test_x):
    Mlp =MLPClassifier(random_state=0, hidden_layer_sizes=210, max_iter=160)
...
def multi_classification(train_X, train_y, df_test, features):
    ...
    for func in ['Linear_SVC','KNN','DecisionTree','RandomForest','XGBoosting','MLP']:
        ...
        fpr, tpr, threshold = roc_curve(test_y, y_pred, pos_label=1)  ###计算真正率和假正率
    ...
    Me.AUC_curve(fprs, tprs, roc_aucs)
        ...
        # avg_metrics = []
        # metric_dict = calc_metrics(test_y, y_pred)
        # print(metric_dict)
        # metric_tuple = [(m, metric_dict[m]) for m in metric_names]
        # print(metric_tuple, sep='\t')
