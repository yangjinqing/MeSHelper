...
import re
import multi_classify_descriptors as cl
...
def data_agg_input():
    ...
    print(data_positive_R.count())
    data_positive_R['re'] = 3
    ...
    train = data[data['year'] <= 2014].loc[:, ['pub_count', 'pub_count_av', 'pub_count_dv','citation_pub', 'citation_av', 'citation_dv', 'count_sib', 'count_dv', 'count_av', 'count_sta','degree', 'closeness', 'clustering', 'pagerank', 'density_rate', 'H2_rate', 'H3_rate', 'ego_number_rate', 're']]
    test = data[data['year'] >= 2015].loc[:, ['type','pub_count', 'pub_count_av', 'pub_count_dv', 'citation_pub', 'citation_av', 'citation_dv', 'count_sib', 'count_dv', 'count_av', 'count_sta', 'degree', 'closeness', 'clustering', 'pagerank', 'density_rate', 'H2_rate', 'H3_rate', 'ego_number_rate', 're']]
    ...
    # test_R = test[test['type'] == 'E']
    # print('test_R:')
    # print(test_R.count())
    # work on data balance, 'H2_rate', 'H3_rate', 'ego_number_rate'
    smote_enn = SMOTEENN(random_state=0)
    smote = SMOTE(random_state=0)
    rus = RandomUnderSampler(random_state=0)
    # print(train[train['re'] == 1]['re'].count())
    X_resampled, y_resampled = smote.fit_resample(train.loc[:, feature], train['re'])
    # f = pd.DataFrame(y_resampled)
    # print(f[f['re'] == 2].count())
    # cl.multi_classification(X_resampled, y_resampled, test, feature)
    print(train.count())
    print(test.count())
